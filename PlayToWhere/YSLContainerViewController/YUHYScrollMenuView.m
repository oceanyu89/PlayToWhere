...
#import "YUHYScrollMenuView.h"

static const CGFloat kYUHYScrollMenuViewWidth  = 90;
static const CGFloat kYUHYScrollMenuViewMargin = 10;
static const CGFloat kYUHYLIndicatorHeight = 3;

@interface YUHYScrollMenuView ()
...
@implementation YUHYScrollMenuView
...
- (void)setItemTitleArray:(NSArray *)itemTitleArray
{
    if (_itemTitleArray != itemTitleArray) {
        ...
        for (int i = 0; i < itemTitleArray.count; i++) {
            CGRect frame = CGRectMake(0, 0, kYUHYScrollMenuViewWidth, CGRectGetHeight(self.frame));
        ...
        _indicatorView.frame = CGRectMake(10, _scrollView.frame.size.height - kYUHYLIndicatorHeight, kYUHYScrollMenuViewWidth, kYUHYLIndicatorHeight);
...
- (void)setIndicatorViewFrameWithRatio:(CGFloat)ratio isNextItem:(BOOL)isNextItem toIndex:(NSInteger)toIndex
{
    ...
    if (isNextItem) {
        indicatorX = ((kYUHYScrollMenuViewMargin + kYUHYScrollMenuViewWidth) * ratio ) + (toIndex * kYUHYScrollMenuViewWidth) + ((toIndex + 1) * kYUHYScrollMenuViewMargin);
    } else {
        indicatorX =  ((kYUHYScrollMenuViewMargin + kYUHYScrollMenuViewWidth) * (1 - ratio) ) + (toIndex * kYUHYScrollMenuViewWidth) + ((toIndex + 1) * kYUHYScrollMenuViewMargin);
    }
    
    if (indicatorX < kYUHYScrollMenuViewMargin || indicatorX > self.scrollView.contentSize.width - (kYUHYScrollMenuViewMargin + kYUHYScrollMenuViewWidth)) {
        return;
    }
    _indicatorView.frame = CGRectMake(indicatorX, _scrollView.frame.size.height - kYUHYLIndicatorHeight, kYUHYScrollMenuViewWidth, kYUHYLIndicatorHeight);
